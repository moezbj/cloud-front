...
import { addRam } from "../../request/admin/ram";
import { getAllMachines } from "../../request/admin/machine";
import useApiState from "../../hooks/useApiState";
import Select from "react-select";
import { generateRandomId } from "utils/generateId";

const AddRamModal = ({ onClose, refetch }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  } = useForm();
  const [ramState, addRamCall] = useApiState(addRam);
  ...
  return (
    <div className="py-4 w-[650px]">
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-2">
        ...
        <div className="input-wrapper flex flex-col my-[10px]">
          <label htmlFor="type_ram" className="text-[16px] text-[#000] pb-4">
            type_ram
          </label>
          <input
            type="text"
